...
SELECT 
    ...
    SUM(order_details.quantity) AS pizza_quantities
FROM
    pizza_types
        JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
ORDER BY pizza_quantities DESC
LIMIT 5;
...
SELECT 
    pizza_types.category,
    (SUM(order_details.quantity * pizzas.price) / (SELECT total_sales FROM cte_1)) * 100 AS revenue
FROM
    pizza_types
    JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
Select name, revenue, category, rn as rank_number
from
